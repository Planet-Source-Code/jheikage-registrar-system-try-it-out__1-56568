...
With rs_Loader
    ...
    If .RecordCount <> 0 Then
        Do
            ...
            MyCRT.Open "Select * FRom Schedules where sn like" & Quoted("%" & SCT & yr & "%") & " and Sc = " & Quoted(.Fields!sc), , adOpenDynamic, adLockOptimistic
            Select Case mode
            Case 0
                'regular
...
x2:
                ...
                End If
            Else
                MyCom = .Fields!sc & "-No Schedule. Needs " & Meetings & " hours."
            End If
            Lbox.AddItem MyCom
            .MoveNext
        Loop Until .EOF
